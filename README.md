...
Run `./deploy.sh` to force-push `main` to `publish`. A GitHub Action then replaces the `?v=@version-...@` placeholders in HTML and JS files with the current commit hash so browsers load updated CSS and JS.
